...
#     try:
#         decoded_token = firebase_admin.auth.verify_id_token(token)
#         user = User(decoded_token['uid'])  # Create Flask-Login user
#         login_user(user)  # Establish Flask-Login session
#         session['firebase_token'] = token  # Store token in session if needed
#         return jsonify({'success': True, 'redirect': url_for('feature')})
#     except Exception as e:
#         return jsonify({'success': False, 'error': str(e)}), 401
...
#         user = User(uid)
#         login_user(user)
#         session['explicit_login'] = True  # Mark as explicit login
#         return jsonify({
#             'success': True,
#             'user': {
#                 'uid': uid,
...
@login_required
def feature():
    if not session.get('explicit_login'):
        return redirect(url_for('auth'))  # Require explicit login
    return render_template('feature.html')

@app.route('/logout')
def logout():
    session.pop('explicit_login', None)
