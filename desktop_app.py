...
import threading
from web import create_app  # Import Flask app creator
...
class Application:
    ...
    def video_loop(self):
        try:
            ...
            if hands and len(hands) > 0:
                ...
                # white = cv2.imread("C:/Users/sonamyadav/Desktop/SonAI_a/white.jpg")
                # # img_final=img_final1=img_final2=0
                white_path = os.path.join(os.path.dirname(__file__), "white.jpg")
                white = cv2.imread(white_path)
                if white is None:
                    white = np.ones((400, 400, 3), dtype=np.uint8) * 255  # Create white image if file not found
    ...
    def predict(self, test_image):
        ...
        # Replace the word suggestion part with:
        if len(self.str.strip()) != 0:
            ...
            self.word = word
            if len(word.strip()) != 0:
            # Use spellchecker instead of enchant
